...
void myGauge3::drawVerticalLines(QPainter *painter)
{
    ...
    for(int i=m_nMin;i<=m_nMax;i++)
    {
        if(i%10==0)
        {
            ...
            qDebug()<<"textWidth:"<<textWidth<<endl;
            qreal textHeight=fontMetrics().height();
            qDebug()<<"textHeight:"<<textHeight<<endl;
            QPointF textPot(startX-textWidth/2,bottomY+textHeight);
            painter->drawText(textPot,strValue);
            qDebug()<<"strValue:"<<strValue<<endl;
...
void myGauge3::drawHandle(QPainter *painter)
{
    ...
    qDebug()<<"NNNN:"<<m_indicatorTopPot<<indicatorLeftPot<<indicatorRightPot<<endl;
